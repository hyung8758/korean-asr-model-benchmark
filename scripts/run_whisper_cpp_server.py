...
LOGGER = logging.getLogger("whisper_cpp_server_launcher")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--server_binary_path", type=Path)
    ...
    parser.add_argument("--jobs", type=int, help="Number of CPU server workers for CPU launcher mode.")
    parser.add_argument("--threads", type=int, help="Number of threads per whisper-server worker.")
    parser.add_argument("--processors", type=int, help="Number of processors per whisper-server worker.")
...
def launch_experiment(config_path: Path, experiment_name: str, gpu_ids: list[int], args: argparse.Namespace) -> None:
    ...
    config = load_config(config_path)
    base_port = args.base_port if args.base_port is not None else int(config.get("base_port", 8100))
    device = args.device or config.get("device", "cuda")
    processes = []
    for shard_index, gpu_id in enumerate(gpu_ids):
        command = [
            sys.executable,
            "scripts/run_whisper_cpp_server.py",
            "--config",
            str(config_path),
            "--experiment",
            experiment_name,
        ...
        add_optional_arg(command, "server_binary_path", args.server_binary_path)
        ...
        add_optional_arg(command, "host", args.host)
        add_optional_arg(command, "language", args.language)
        add_optional_arg(command, "limit", args.limit)
        add_optional_arg(command, "threads", args.threads)
        add_optional_arg(command, "processors", args.processors)
        if args.no_resume:
            command.append("--no_resume")
        if args.retry_errors:
            command.append("--retry_errors")
        LOGGER.info("Launching shard %s on %s worker=%s port=%s for %s", shard_index, device, gpu_id, base_port + shard_index, experiment_name)
        processes.append((shard_index, subprocess.Popen(command, cwd=PROJECT_ROOT)))

    wait_for_processes(processes, experiment_name)


def launcher_worker_ids(args: argparse.Namespace, config: dict) -> list[int]:
    device = args.device or config.get("device", "cuda")
    if device != "cpu":
        return parse_gpu_ids(args, config)
    if args.jobs is not None:
        return list(range(args.jobs))
    num_jobs = int(config.get("launcher", {}).get("num_jobs", 1))
    return list(range(num_jobs))


def launch_all_experiments(args: argparse.Namespace) -> None:
    config = load_config(args.config)
    worker_ids = launcher_worker_ids(args, config)
    device = args.device or config.get("device", "cuda")
    if device == "cpu":
        validate_launcher_args(args, worker_ids)
    else:
        validate_gpu_launcher_args(args, worker_ids)
    apply_result_root_override(config, args)
    setup_launcher_logging(config)
    LOGGER.info("Launcher mode: %s experiments, device=%s workers=%s", len(config["experiments"]), device, worker_ids)

    for experiment in config["experiments"]:
        experiment_name = experiment["name"]
        LOGGER.info("Starting experiment: %s", experiment_name)
        launch_experiment(args.config, experiment_name, worker_ids, args)
...
def main() -> None:
    ...
    if args.worker or args.experiment:
        run_configured_experiment(args, apply_overrides, build_run_config, run_whisper_cpp_server)
        return
